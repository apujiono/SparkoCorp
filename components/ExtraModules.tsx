    ...
    return (
        <div className="p-6 h-full overflow-y-auto pb-20 bg-[#05010a] text-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="glass-panel p-6 rounded-xl border border-purple-900/50">
                    <p className="text-xs text-slate-500 uppercase">Days Without Incident</p>
                    <p className="text-4xl font-bold font-mono mt-2 text-purple-400">14 Days</p>
                </div>
...
export const TrainingModule: React.FC<{
    ...
    return (
        <div className="p-6 h-full overflow-y-auto pb-20 bg-[#05010a] text-slate-200">
             <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                    <GraduationCap className="text-purple-500" /> Training Academy
                </h2>
                <button onClick={() => setShowModal(true)} className="bg-purple-600 hover:bg-purple-500 px-4 py-2 rounded text-white text-sm font-bold flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {courses.map(course => (
                    <div key={course.id} className="glass-panel p-6 rounded-xl border border-purple-900/30 hover:border-purple-500 transition group">
                        <div className="flex justify-between items-start mb-4">
                            <div className="p-3 bg-purple-900/20 rounded-lg text-purple-400 group-hover:bg-purple-600 group-hover:text-white transition">
                                <BookOpen size={24}/>
                            </div>
                        ...
                        <button className="w-full py-2 bg-slate-800 hover:bg-purple-600 hover:text-white rounded text-sm transition">Manage Enrollment</button>
                    </div>
                ))}
            </div>
             ...
             {showModal && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
                    <div className="glass-panel p-6 rounded-xl w-96">
                        ...
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setShowModal(false)} className="text-slate-400">Cancel</button>
                            <button onClick={handleAdd} className="bg-purple-600 px-4 py-2 rounded text-white">Create</button>
                        </div>
                    </div>
                </div>
            )}
    ...
    return (
        <div className="p-6 h-full overflow-y-auto pb-20 bg-[#05010a] text-slate-200">
             <h2 className="text-2xl font-bold text-white flex items-center gap-2 mb-8">
                <BarChart2 className="text-orange-500" /> Business Intelligence
            </h2>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <RePie data={data} dataKey="profit" nameKey="name" cx="50%" cy="50%" outerRadius={80}>
                            {data.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={['#9333ea', '#f97316', '#db2777', '#8b5cf6'][index % 4]} />
                            ))}
                            <Tooltip contentStyle={{backgroundColor: '#0f172a', borderColor: '#334155'}} />
                        </RePie>
                    </ResponsiveContainer>
                             ...
                             {data.map((row, i) => {
                                 ...
                                 return (
                                     <tr key={i} className="hover:bg-slate-900/50">
                                         ...
                                         <td className="p-3 text-orange-400 font-bold">Rp {row.profit.toLocaleString()}</td>
                                         <td className="p-3">
                                             <span className={`px-2 py-1 rounded text-xs ${margin > 20 ? 'bg-purple-900/30 text-purple-400' : 'bg-yellow-900/30 text-yellow-400'}`}>
                                                 {margin.toFixed(1)}%
                                             </span>
                                         </td>
                                     </tr>
